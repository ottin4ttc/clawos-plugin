...
import { DAEMON_BASE_URL, COMMAND_TIMEOUT, generateId } from "@clawos/shared";
import type { Request, Response } from "@clawos/shared";
...
const EXT_HINT = [
  ...
  "",
  "1. Download extension: https://github.com/ottin4ttc/clawos-plugin/releases/latest",
...
async function sendCommand(request: Request): Promise<Response> {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    return { id: request.id, success: false, error: "Failed to start daemon. Run manually: clawos daemon" };
  }
}
...
const server = new McpServer(
  { name: "clawos", version: "0.4.0" },
  { instructions: `clawos lets you control the user's real Chrome browser — with their login state, cookies, and sessions.
...
- Run via CLI: clawos site <name> [args]
- Available: reddit, twitter, github, hackernews, xiaohongshu, zhihu, bilibili, weibo, douban, youtube
- Update: clawos site update
- List all: clawos site list

To create a new site adapter, run: clawos guide` },
);
