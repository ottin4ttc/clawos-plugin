 ...
 * 用法：
 *   clawos tab                    列出所有标签页
 *   clawos tab new [url]          新建标签页
 *   clawos tab <n>                切换到第 n 个标签页（按 index）
 *   clawos tab close [n]          关闭标签页（按 index）
 *   clawos tab select --id <id>   切换到指定 tabId 的标签页
 *   clawos tab close --id <id>    关闭指定 tabId 的标签页
 */
...
function parseTabSubcommand(args: string[], rawArgv?: string[]): {
  ...
  if (first === "select") {
    ...
    throw new Error("tab select 需要 --id 参数，用法：clawos tab select --id <tabId>");
  }
